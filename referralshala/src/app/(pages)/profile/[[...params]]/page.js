 ...
 let data='';
  const fetchuserdata= async ()=>{
    try {
      ...
      if (data.success) {
        console.log("Referral created successfully", data.data);

        const userData = data.data;

        // Update tempData state with the fetched user data
        setTempData((prevData) => ({
          ...prevData,
          name: `${userData.firstName} ${userData.lastName}`}));
      } else {
        console.error("Failed to create referral", data.message);
      }
    } catch (error) {
